...
} from "../api/operationalEscalationMatrixAPI";
...
const Operational = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getAllOperationalEscalationMatrix();
        setEscalationMatrix(data);
      } catch (error) {
        console.error("Error fetching Operational escalation matrix:", error);
      }
    };
    fetchData();
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const newEntry = await createOperationalEscalationMatrix(formData);
      setEscalationMatrix([...escalationMatrix, newEntry]);
      setFormData({ project_id: "", level: "", name: "" });
    } catch (error) {
      console.error("Error adding Operational escalation matrix:", error);
    }
  };
  ...
  const handleEdit = async (id) => {
    try {
      ...
      console.error("Error editing Operational escalation matrix:", error);
    }
  };

  const handleSaveEdit = async () => {
    try {
      await updateOperationalEscalationMatrix(editingId, editFormData);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteOperationalEscalationMatrix(id);
      setEscalationMatrix(escalationMatrix.filter((entry) => entry._id !== id));
    } catch (error) {
      console.error("Error deleting Operational escalation matrix:", error);
    }
  };
      ...
      <Layout>
        <Grid container justifyContent="center">
          {(role === "projectmanager" || role === "admin") && (
            <Grid item xs={10}>
              <h2>Add Operational Escalation Matrix</h2>
            </Grid>
          )}
          {(role === "projectmanager" || role === "admin") && (
            <Grid item xs={10}>
        ...
        >
          ...
          >
            <Typography variant="h6" id="edit-modal-title" sx={{ mb: 2 }}>
              Edit Operational Escalation Matrix
            </Typography>
