...
const Operational = () => {
  ...
  useEffect(() => {
    if (id && projects) {
      ...
      if (project) {
        const { operationalMatrix } = project;
        setEscalationMatrix(operationalMatrix);
      }
    }
  }, [id, projects]);
  ...
  const handleDelete = async (entryId) => {
    try {
      ...
      setAuthHeader(token);
      await axiosInstance.put(`/project/edit/${id}`, {
        operationalMatrix: escalationMatrix.filter(
