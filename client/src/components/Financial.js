...
} from "../api/financialEscalationMatrixAPI";
...
const Financial = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getAllFinancialEscalationMatrix();
        setEscalationMatrix(data);
      } catch (error) {
        console.error("Error fetching Financial escalation matrix:", error);
      }
    };
    fetchData();
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const newEntry = await createFinancialEscalationMatrix(formData);
      setEscalationMatrix([...escalationMatrix, newEntry]);
      setFormData({  level: "", name: "" });
    } catch (error) {
      console.error("Error adding Financial escalation matrix:", error);
    }
  };
  ...
  const handleEdit = async (id) => {
    try {
      ...
      console.error("Error editing Financial escalation matrix:", error);
    }
  };

  const handleSaveEdit = async () => {
    try {
      await updateFinancialEscalationMatrix(editingId, editFormData);
  ...
  const handleDelete = async (id) => {
    try {
      await deleteFinancialEscalationMatrix(id);
      setEscalationMatrix(escalationMatrix.filter((entry) => entry._id !== id));
    } catch (error) {
      console.error("Error deleting Financial escalation matrix:", error);
    }
  };
      ...
      <Layout>
        <Grid container justifyContent="center">
          {(role === "projectmanager" || role === "admin") && (
            <Grid item xs={10}>
              <h2>Add Financial Escalation Matrix</h2>
            </Grid>
          )}
          {(role === "projectmanager" || role === "admin") && (
            <Grid item xs={10}>
        ...
        >
          ...
          >
            <Typography variant="h6" id="edit-modal-title" sx={{ mb: 2 }}>
              Edit Financial Escalation Matrix
            </Typography>
